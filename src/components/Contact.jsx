...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    emailjs.send("service_4yje7lo", "template_x8cu2mn", {from_name: form.name, to_name: 'Akram', from_email: form.email, to_email: 'akramhelali007@gmail.com', message: form.message}, '0lDkFrvpXQL2Czphy').then(() => {setLoading(false); alert('Thank you. I will get back to you as soon as possible.'); setForm({name:'', email:'', message:'',})}, (error) => {setLoading(false); console.log(error); alert("Something went wrong.")})

  }
  return (
    <div className="xl:mt-0 xl:flex-row flex-col-reverse flex gap-10 overflow-hidden">
      <motion.dev variants={slideIn('left', 'tween', 0.2, 1)} className='flex-[0.75] bg-black-100 p-8 rounded-2xl'>
